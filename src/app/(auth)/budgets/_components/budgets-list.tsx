    ...
    if (rows.length === 0) {
        return (
            <div className="border border-dashed p-10 text-center text-xs text-muted-foreground">
                No categories yet. Create a category first to budget for it.
            </div>
        );
    }
    ...
    return (
        <div className="overflow-hidden rounded-md border text-xs">
            <div className="flex items-center justify-between border-b px-3 py-2">
                ...
                <span>
                    <span className="font-medium">{fmt.format(total.spent)}</span>
                    <span className="text-muted-foreground"> of {fmt.format(total.budget)}</span>
                </span>
            </div>
            <ul>
                {rows.map((row) => {
                            ...
                            <div className="min-w-0 space-y-1">
                                <div className="flex items-baseline justify-between gap-2">
                                    ...
                                    <span>
                                        ...
                                        {budget != null && (
                                            <span className="text-muted-foreground"> / {fmt.format(budget)}</span>
                                        )}
                                    </span>
                                </div>
                                {budget != null ? (
